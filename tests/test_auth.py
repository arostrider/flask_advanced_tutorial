...
def test_register(client, app):
    ...
    response = client.post(
        "/auth/register", data={"username": "a", "password": "a", "confirm password": "a"}
    )
...
@pytest.mark.parametrize(("username", "password", "confirm_password", "message"), (
        ...
        ("a", "a", "", b"Password and password confirmation must match."),
        ("test", "test", "test", b"already registered.")
))
def test_register_validate_input(client, username, password, confirm_password, message):
    response = client.post(
        "/auth/register",
        data={"username": username, "password": password, "confirm password": password}
    )
